...
const Monogram = ({ className='' }) => (
  <svg viewBox="0 0 200 200" className={className}>
    ...
    <motion.circle cx="100" cy="100" r="78" fill="none" stroke="url(#gold)" strokeWidth="1.2"
...
export default function WeddingInvitePreview(){
    ...
    <div className='mb-10'>
      ...
      {open===title && (
        <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}}
          transition={{duration:0.7,ease:'easeInOut'}} className='overflow-hidden mt-5 rounded-[2rem] shadow-2xl'>

          <div className='min-h-[600px] rounded-[2rem] p-6 flex flex-col justify-end bg-cover bg-center relative overflow-hidden'
            style={{backgroundImage:`linear-gradient(to bottom, rgba(15,81,50,0.2), rgba(15,81,50,0.5), rgba(15,81,50,0.95)), url(${img})`}}>

            <motion.div className='absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent'
              animate={{ x: ['-100%','200%'] }} transition={{ repeat: Infinity, duration: 8 }} />

            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className='relative'>
              <h3 className='text-4xl mb-3' style={{fontFamily:'Book Antiqua', color:'#F2D28F'}}>{title}</h3>
              <p style={{fontFamily:'Times New Roman'}}>{date}</p>
              <p style={{fontFamily:'Times New Roman'}}>{venue}</p>

              <div className='flex gap-3 mt-6 flex-wrap'>
                <a href={mapLink} target='_blank' className='border border-[#C9A86A] rounded-full px-4 py-2'>Directions</a>

                {showUpload && (
                  <a href={uploadLink} target='_blank' className='border border-slate-300 rounded-full px-4 py-2'>Upload Photos</a>
                )}

                {showGallery && (
                  <a href={galleryLink} target='_blank' className='bg-[#C9A86A] text-black rounded-full px-4 py-2'>View Gallery</a>
                )}
              </div>
            </motion.div>
          </div>
          ...
          <div className='p-4 bg-[#0F5132]'>
            <iframe src={embed} width="100%" height="250" style={{border:0}} loading="lazy" />
          </div>
        </motion.div>
      )}
      </AnimatePresence>
  ...
  return (
    <div className='min-h-screen text-white overflow-x-hidden'>
      ...
      <AnimatePresence>
        {!loaded && (
          <motion.div className='fixed inset-0 z-50 flex items-center justify-center bg-[#0F5132]' exit={{opacity:0}}>
            <div className='w-40 h-40'><Monogram /></div>
          </motion.div>
        )}
      </AnimatePresence>

      <section className='min-h-screen flex items-center justify-center px-6 relative text-center overflow-hidden'
        style={{backgroundImage:`linear-gradient(rgba(15,81,50,0.7),rgba(15,81,50,0.95)), url("/Monsoon Background.png")`, backgroundSize:'cover'}}>

        <motion.div className='absolute w-[400px] h-[400px] bg-white/5 blur-3xl'
          animate={{ x:[0,40,0], y:[0,20,0] }} transition={{ repeat: Infinity, duration: 20 }} />

        <div className='relative z-10'>
          <div className='w-24 h-24 mx-auto mb-6'><Monogram /></div>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            className='text-5xl' style={{fontFamily:'Book Antiqua'}}>Ashfaq weds Afreen</motion.h1>
